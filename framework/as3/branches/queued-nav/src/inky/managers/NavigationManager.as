package inky.managers
{
	...
	import inky.managers.LoadManager;
	...
	public class NavigationManager
	{
		 ...
		 *     section's URL.
		 * @see inky.core.Section#initialize()
		 ...
		 *	
		 * @see inky.core.NavigationManager#gotoSection()
		 * @see inky.core.Section#gotoSection()
		 *	
		 * @param target
		 *     The section to which to navigate, as represented by an SPath
...
// TODO: Is this required to clear the progress bars after Application and data load?
//this._cmdQueue.push({type: '__removeItemProgressBar'});
//this._cmdQueue.push({type: '__removeCumulativeProgressBar'});
			...
			for (i = index; i < sPath.length; i++)
			{
				...
				this._cmdQueue.push({type: '__preload'});
				this._cmdQueue.push({type: '__removeItemProgressBar'});
				this._cmdQueue.push({type: '__removeCumulativeProgressBar'});
...
// TODO: Remove listeners on first item?
		...
		/**
		 *
		 * Removes the item progress bar from the current section.	
		 *	
		 */
		private function __removeItemProgressBar(cmd:Object):void
		{
			var section:Section = this._getLeafSection();
			if (section.itemProgressBar && section.itemProgressBar.parent)
			{
				this._addCommandCompleteListener(cmd, section, 'removeProgressBarComplete');
				section.removeItemProgressBar(section.itemProgressBar);
			}
			else
			{
		...
		/**
		 *
		 * Removes the cumulative progress bar from the current section.	
		 *	
		 */
		private function __removeCumulativeProgressBar(cmd:Object):void
		{
			var section:Section = this._getLeafSection();
			if (section.cumulativeProgressBar && section.cumulativeProgressBar.parent)
			{
				this._addCommandCompleteListener(cmd, section, 'removeProgressBarComplete');
				section.removeCumulativeProgressBar(section.cumulativeProgressBar);
			}
			else
			{
				this._commandCompleteHandler();
		...
		private function __preload(cmd:Object):void
		{
			...
			loadManager.preload(this._getLeafSection(), this._currentSPath);
		}
