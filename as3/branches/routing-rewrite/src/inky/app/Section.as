package inky.app
{
	...
	import inky.app.SectionOptions;
	...
	import inky.app.managers.NavigationManager;
     ...
     *
     * @eventType inky.app.events.SectionEvent.NAVIGATION_COMPLETE
     *
     */
	[Event(name="navigationComplete", type="inky.app.events.SectionEvent")]
...
Section.setSection(this.__itemLoadingProgressBar, this.sPath || '/');
		 ...
		 * @see #fetchAsset()
		 * @see #fetchAssetNow()
		 * @see inky.loading.loaders.IAssetLoader#close()
		 *
		 */		 		 		 		
		 ...
		 * @see inky.loading.loaders.IAssetLoader#load()
		 * 
		 */
		 ...
		 * @see #fetchAsset()
		 * @see #cancelFetchAsset()
		 * @see inky.loading.loaders.IAssetLoader#loadNow()
		 * 
		 */
		 ...
		 *
		 */
		public function gotoURL(url:Object, target:String = null):void
		{
			if (this == this.master)
			{
				var request:URLRequest;
				...
				this.navigateToURL(request, target);
			}
			else
			{
				this.owner.gotoURL(url, target);
			}
		}
...
// TODO: is this necesssary anymore??
		...
		private function _destroy():void
		{
			...
			// Remove sectionOptions callbacks
		...
		private function _init():void
		{
			...
			//
			if (getQualifiedClassName(this) == 'inky.app::Section')
			{
