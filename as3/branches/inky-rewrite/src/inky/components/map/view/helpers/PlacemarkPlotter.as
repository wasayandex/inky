package inky.components.map.view.helpers 
{
	 ...
	 *
	 */
	public class PlacemarkPlotter implements IDestroyable
	{
		...
		/**
		 * Creates a new PlacemarkPlotter.
		 * 
		 * @param map
		...
		public function PlacemarkPlotter(map:IMap, recyclePlacemarkRenderers:Boolean = true, cachePlacemarkPositions:Boolean = true)
		{
			...
			var contentContainer = DisplayObjectContainer(map).getChildByName("_contentContainer") as Sprite;
			if (!contentContainer)
			{
				contentContainer = new Sprite();
				DisplayObjectContainer(map).addChild(contentContainer);
			}
	...
	this.placemarks2Renderers[placemark] = renderer;
		...
		protected function validate():void
		{
			...
			if (placemarksAreInvalid)
			{
				...
				while (this.contentContainer.numChildren)
...
//					this.adjustScaleForPlacemark(placemark as DisplayObject);
					this.contentContainer.addChild(placemark as DisplayObject);
				}
			}
		}
