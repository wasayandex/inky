...
import flash.utils.*;
		...
		{
//trace("GraphicLoader._completeHandler hears Loader COMPLETE for " + e.currentTarget.url.split("/").slice(-4).join("/"));
			
this.setContent(this._loader.content);
this.dispatchEvent(new AssetLoaderEvent(AssetLoaderEvent.READY));

/*//trace('\tloaded ' + this['getTheLoader']().contentLoaderInfo.url.split('/').slice(-4).join('/'));
if (this['getTheLoader']().contentLoaderInfo.url.match(this.source))
{
//	this.setContent(this._loader.content);
_to =	setTimeout(_f, 0);
//trace("GraphicLoader dispatches READY");
//	this.dispatchEvent(new AssetLoaderEvent(AssetLoaderEvent.READY));
}
else
{
//	trace('\t\t1\t' + this['getTheLoader']().contentLoaderInfo.url.split('/').slice(-4).join('/') + ' != ' + this.source);
}*/
		}
...
private function _k(e)
{
//	trace("Loader dispatches COMPLETE for " + e.currentTarget.url.split("/").slice(-4).join("/"));
}
...
var url:String = e.currentTarget.url ? e.currentTarget.url.split("/").slice(-4).join("/") : null;
if (e.type == "complete")
//trace("GraphicLoader._relayEvent hears Loader " + e.type.toUpperCase() + " for " + url + "\tsource: " + this.source);

this._behavior.relayEvent(e);

/*if (this['getTheLoader']().contentLoaderInfo.url)
{
	if (this['getTheLoader']().contentLoaderInfo.url.match(this.source))
	{
		if (e.type=="complete")
	...
	{
	trace(".........................................................")
		import inky.framework.components.gallery.views.*;
			var whatever = GalleryItemView.whatever;
		whatever.addChild(this['getTheLoader']());
		import com.exanimo.layout.*;
		new GridLayout(10).layoutContainer(whatever);	
	}
}*/
		}
